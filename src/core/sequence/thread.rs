...
use crate::core::{
    part::part::Part, sample::SampleCategory, sequence::sequencer::Sequencer, synth::Synth,
};
...
impl SequencerThread {
    ...
    pub fn create_thread(&self, synth: Arc<RwLock<Synth>>, sample_category: &SampleCategory) {
        let clone_sequencer = Arc::clone(&self.sequencer);
        let clone_option = Arc::clone(&self.option);
        let clone_parts = Arc::clone(&self.parts);
        let clone_sample_category = sample_category.clone();
        ...
        std::thread::spawn(move || {
            loop {
                ...
                for (id, part) in parts.iter().enumerate() {
                    ...
                    let dirty_sample = synth.parts[id].param.sample != param.sample;
                    synth.parts[id].param = param.clone();

                    if dirty_sample {
                        let _ = synth.parts[id].load_sample(&clone_sample_category);
                    }
                ...
                let mut next_step_duration = 60.0 / option.bpm / 4.0;
